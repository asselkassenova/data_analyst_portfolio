...
df <- read_csv("C:\\Users\\pouro\\Downloads\\Bank_Personal_loan_Modelling.csv")
dim(df)
...
# The problem statement for this dataset is to identify factors that influence customers' 
# acceptance of personal loans and develop a predictive model to target potential loan customers.
...
df3 %>%
  ...
  labs(title = "Education Distribution by Personal Loan Status", x = "Education", y = "Count", fill = "Personal Loan")
...
df3 %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Income Distribution by Personal Loan Status", x = "Income", y = "Count", fill = "Personal Loan") +
  scale_fill_manual(values = c("0" = "#FC4E07", "1" = "#00AFBB"), name = "Personal Loan", labels = c("Not Accepted", "Accepted"))
...
#higher interest rates, which could make the loan less attractive to them. On the other hand, customers with higher incomes 
#may be offered lower interest rates, which could make the loan more attractive to them
...
#A securities account typically does not have a direct impact on whether a person 
#is approved or denied for a personal loan. We can consider not including this variable in our model. 
...
df3 %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Credit Card Distribution by Personal Loan Status", x = "Credit Card", y = "Count", fill = "Personal Loan") +
...
# Additionally, customers who do not have a credit card may simply be less 
#interested in borrowing money or may prefer to use other means of financing 
#  One possible explanation could be that individuals who do not have a credit card may have a lower credit score, which may make it harder for them 
#to get approved for loans from other financial institutions. Thus, they may be more likely to accept a loan offer from the bank even if the interest 
#rates are higher
# On the other hand, individuals with credit cards may have a higher credit score and may have access to other loan options with better terms and lower interest rates, 
#making them less likely to accept the loan offer from the bank.
...
df3 %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "CCAvg Distribution by Personal Loan Status", x = "CCAvg", y = "Count", fill = "Personal Loan") +
  scale_fill_manual(values = c("Not Accepted" = "#FC4E07", "Accepted" = "#00AFBB"))


# Those who did not accept has a lower CCavg:Average spending on credit cards per month. 
#This relationship may indicate that customers who spend more on their credit cards may have a higher income or a better credit score, 
#which makes them more eligible for a personal loan offer. Alternatively, it may be that customers who accepted the personal loan offer 
#did so in order to pay off their credit card debt, which could explain the higher credit card spending. 
...
df3 %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Mortgage by Personal Loan Status", x = "CCAvg", y = "Count", fill = "Personal Loan") +
  scale_fill_manual(values = c("Not Accepted" = "#FC4E07", "Accepted" = "#00AFBB"))
...
df3 %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Family Size by Personal Loan Status", x = "Family", y = "Count", fill = "Personal Loan") +
  scale_fill_manual(values = c("Not Accepted" = "#FC4E07", "Accepted" = "#00AFBB"))
...
df3 %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Age by Personal Loan Status", x = "Age", y = "Count", fill = "Personal Loan") +
  scale_fill_manual(values = c("Not Accepted" = "#FC4E07", "Accepted" = "#00AFBB"))
...
df3 %>%
  ...
  geom_bar(stat = "identity", position = "dodge") +
  labs(title = "Years Experience by Personal Loan Status", x = "Experience", y = "Count", fill = "Personal Loan") +
  scale_fill_manual(values = c("Not Accepted" = "#FC4E07", "Accepted" = "#00AFBB"))
...
# Overall, the scatterplot matrix suggests that there may not be strong linear relationships between the independent 
#variables and the target variable such that multicollinearity is present. 
...
#Seems to be imba;anced towards predicting not accept outcome. 
...
#This insight can help businesses understand the main drivers behind customers' decision-making process and focus their 
#efforts on those factors to improve loan acceptance rates.
# Key Factors Identified:
# Income level, Credit History, employment history, Age and experience. 


#2: Customer Segmentation:
#Customer Segmentation: The analysis can uncover different segments of 
#customers based on their likelihood of accepting personal loans. 
#By understanding these segments, businesses can tailor their marketing and
#communication strategies to target each segment effectively. 
#This segmentation can be based on various characteristics such as demographics,
#income levels, employment status, or credit history. 

#3: Predictive Model: Developing a predictive model allows businesses to forecast the likelihood of a customer accepting
#a personal loan. 
# Best Model we suggest is the logistic regression model which best suits this particular data set as well as our problem statement.
#enables proactive targeting of potential loan customers, leading to more effective 
#marketing campaigns and resource allocation. 
#By accurately identifying potential loan customers, businesses can streamline their efforts and improve conversion rates.

#4: Risk Assessment: 
#The analysis can also provide insights into the risk associated with granting personal loans to different customers. 
#By analyzing historical data and customer attributes, businesses can assess the creditworthiness of potential borrowers and make informed 
#decisions to mitigate risks. This insight helps optimize the loan approval process and minimize default rates.
# This is something to consider for future use of this analysis as it currently predicts loan acceptance and not loan approval. 

#5: Product Development and Offerings:
#Understanding the factors that influence customers' acceptance of personal loans can guide product development efforts.
#By identifying customer preferences, pain points, and specific needs, businesses can tailor their loan products and offerings 
#to better align with customer expectations. 
#This insight can help differentiate their loan products from competitors and enhance customer satisfaction.
...
#By identifying influential factors, businesses can prioritize marketing channels, messages, 
#and promotional activities that resonate with potential loan customers.
#This insight helps allocate marketing budgets more effectively, resulting in higher customer 
#engagement and conversion rates.


# Please see our visualization dashboard :)
